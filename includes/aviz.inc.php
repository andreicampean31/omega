...
if ( isset ( $_POST['aviz_id'] ) )
   {
      ...
      //check firma
      foreach($_POST['aviz_id'] as $id_check){
        $sql_aviz_check_firma = "SELECT firma FROM comanda WHERE id='$id_check'";
        $result_aviz_check_firma = mysqli_query($conn, $sql_aviz_check_firma);
        while($row_aviz_check_firma = mysqli_fetch_array($result_aviz_check_firma)){
          $firma_check = $row_aviz_check_firma['firma'];
          echo $firma_check. "</br>";
        }
      }
      ...
      foreach ( $_POST[ 'aviz_id' ] as $aviz_id )
         {
           ...
           $id = $aviz_id;
            ...
            while($row_aviz_comanda = mysqli_fetch_array($result_aviz_comanda)){
              if($row_aviz_comanda['firma'] != $firma_check){
                echo"error";
                $error = 1;
                header("Location: ../comanda.php?error=firma_diferita&sortare=toate");
              }
              else{
                echo "not error";
